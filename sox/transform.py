...
class Transformer(object):
    ...
    def noiseprof(self, profile='noise.prof'):
        ...
        noisered
        '''
        effect_args = ['noiseprof', profile]

        self.effects.extend(effect_args)
        self.effects_log.append('noiseprof')
        return self

    def noisered(self, amount=0.5, profile='noise.prof'):
        ...
        profile: str, default='noise.prof'
            The noise data generated by noiseprof
        '''
        if not isinstance(amount, float):
            raise ValueError("amount must be a float number.")

        file_info.validate_input_file(profile)
